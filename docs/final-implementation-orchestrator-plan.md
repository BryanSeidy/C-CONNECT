...
The concrete four-agent delivery split is maintained in `docs/repartition-implementation-4-agents.md`. That execution repartition maps Claude1 to authentication, Zai to backend business workflows, Claude2 to frontend MVP/UX completion, and the current CTO agent to integration, CI, production hardening and final go/no-go.
