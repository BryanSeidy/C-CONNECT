...
- [x] **Critical bug (backend, found this session):** `users` table was missing `name`, `fullName`, `companyName`, `country`, `isVerified` columns that `AuthController::register`, `RfqController`, `DisputeController`, and `RecurringOrderController` all read/write — meaning **every registration attempt would throw a fatal SQL error** and RFQ/Dispute/Recurring-order listings would 500 on their eager-load selects. `nom`/`prenom` were also NOT NULL with no default and nothing populated them anymore. New migration `2026_07_08_000001_fix_users_missing_columns.php` adds the missing columns and relaxes `nom`/`prenom` to nullable. Not yet run against a real DB (no Postgres access in the sandbox) — **run `php artisan migrate` and smoke-test registration before demo.**
